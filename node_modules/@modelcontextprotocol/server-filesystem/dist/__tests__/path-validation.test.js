...
describe('Path Validation', () => {
    ...
    describe('Error handling', () => {
        ...
        it('handles null and undefined inputs gracefully', () => {
            ...
            expect(isPathWithinAllowedDirectories(null, allowed)).toBe(false);
            expect(isPathWithinAllowedDirectories(undefined, allowed)).toBe(false);
            expect(isPathWithinAllowedDirectories('/path', null)).toBe(false);
            expect(isPathWithinAllowedDirectories('/path', undefined)).toBe(false);
        });
    });
    ...
    describe('Edge Cases', () => {
        it('rejects non-string inputs safely', () => {
            const allowed = ['/home/user/project'];
            expect(isPathWithinAllowedDirectories(123, allowed)).toBe(false);
            expect(isPathWithinAllowedDirectories({}, allowed)).toBe(false);
            expect(isPathWithinAllowedDirectories([], allowed)).toBe(false);
            expect(isPathWithinAllowedDirectories(null, allowed)).toBe(false);
            expect(isPathWithinAllowedDirectories(undefined, allowed)).toBe(false);
            // Non-string in allowed directories
            expect(isPathWithinAllowedDirectories('/home/user/project', [123])).toBe(false);
            expect(isPathWithinAllowedDirectories('/home/user/project', [{}])).toBe(false);
        });
    ...
    describe('Path Validation Race Condition Tests', () => {
        ...
        it('should prevent race condition between validatePath and file operation', async () => {
            ...
            await expect(fs.writeFile(racePath, 'NEW CONTENT', { encoding: 'utf-8', flag: 'wx' })).rejects.toThrow(/EEXIST/);
